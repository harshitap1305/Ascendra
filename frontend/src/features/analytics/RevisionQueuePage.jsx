    ...
    <AppLayout>
      <div className="max-w-2xl mx-auto">
        <div className="flex items-start justify-between mb-6 flex-wrap gap-4">
          <div>
            <Link to={`/exams/${examId}/dashboard`} className="text-slate-500 hover:text-slate-300 text-sm mb-1 block transition-colors">
              ← Dashboard
            </Link>
            <h1 className="text-2xl font-bold text-white">Revision Queue</h1>
            <p className="text-slate-400 text-sm mt-1">Spaced revisions — {queue.length} due today</p>
          </div>
        </div>
        ...
        {loadingQueue ? (
          ...
          <div className="card text-center py-12">
            ...
            <h3 className="text-slate-200 font-semibold mb-2">All caught up!</h3>
            <p className="text-slate-500 text-sm">No revisions due today. Keep studying and revisions will appear here when topics are due.</p>
          </div>
        ) : (
          ...
          <div className="mt-8">
            ...
            {loadingUpcoming ? null : (
              <div className="space-y-2">
                {upcoming.map(r => (
                  <div key={r.id} className="flex items-center justify-between py-2.5 px-4 bg-slate-900 rounded-lg border border-slate-800">
                    <div>
                      <span className="text-slate-300 text-sm font-medium">{r.topic_name}</span>
        ...
        <div className="mt-8 card bg-slate-900/50">
          <h3 className="text-slate-300 text-sm font-semibold mb-2">📖 How Spaced Repetition Works</h3>
          <p className="text-slate-500 text-xs leading-relaxed">
            When you complete a topic, we automatically schedule 5 revision sessions at increasing intervals:
            <span className="text-slate-400"> 1 day → 3 days → 7 days → 15 days → 30 days.</span>
